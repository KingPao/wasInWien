...
- **Open-Air-Kinos & Kino**: VOLXkino/KARLSkino (`volxkino.at`, wandert im Sommer zwischen Karlsplatz und anderen Plaetzen) und Filmarchiv Austria "Kino wie noch nie" + METRO Kinokulturhaus (`filmarchiv.at`) sind beide direkt server-seitig gerendert und liefern echte, datierte Vorstellungen. filmarchiv.at zeigt Termine ohne Jahreszahl (z.B. "10.7., 21:30") -- dafuer wurde der Collector um eine Jahres-Heuristik erweitert (aktuelles Jahr, ausser das Datum laege dann mehr als 30 Tage in der Vergangenheit, dann naechstes Jahr). Andere Sommerkinos (Kino am Dach, Film Festival Rathausplatz, Science Fiction im Park, Stumm & Laut, frame[o]ut) haben keine eigene serverseitig gerenderte Terminliste bzw. keine passende wasmachma.at-Seite gefunden -- koennten spaeter einzeln ergaenzt werden, sobald eine scrapbare Quelle gefunden ist.
- **Clubs, Live-Venues & Theater (Flex, Fluc, Grelle Forelle, Das Werk, B72, WUK, Celeste, Szene Wien, Arena Wien, Globe Wien/Marx Halle, Peterskirche, Ronacher, STADTSAAL, Haus der Musik, Spanische Hofreitschule, Raimund Theater, Kabarett Niedermair, Kulisse, Karlskirche, Rabenhof Theater, Orpheum Wien, Musikverein Wien, Peterskirche Krypta)**: die Venues selbst sind teils nicht zuverlaessig erreichbar bzw. JS-lastig. Alle haben aber echte, datierte Terminlisten auf **wasmachma.at** (einem groesseren oesterreichischen Event-Aggregator), server-seitig als Tabelle gerendert -- deshalb jeweils ueber `wasmachma.at/veranstaltungsort/<slug>` eingebunden. Zusaetzlich als breiter Catch-all: `wasmachma.at/bundesland/wien/events` (die allgemeine Wien-Eventliste der Plattform, deckt auch Venues ab, die nicht einzeln eingebunden sind). Der Link-Pattern-Collector wurde dafuer erweitert, damit er Datum/Bild auch aus Tabellenzeilen (`<tr>`) statt nur `<article>/<li>/<div>` liest. Weitere Venues auf `wasmachma.at/bundesland/wien/veranstaltungsorte` koennen nach demselben Muster ergaenzt werden.
